...
class NRBaseStation:
    ...
    def compute_nprb_NR(self, data_rate, rsrp):
        ...
        thermal_noise = constants.Boltzmann*293.15*list(NRbandwidth_prb_lookup[self.numerology][self.fr].keys())[list(NRbandwidth_prb_lookup[self.numerology][self.fr].values()).index(self.total_prb / (10 * 2**self.numerology))]*1000000*self.compute_rbur()
